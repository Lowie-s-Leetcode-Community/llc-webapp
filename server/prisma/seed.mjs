...
async function main() {
  const { default: quizzesList } = await import(
    "./backup_json_data/discord_quiz.json",
    {
      assert: {
        type: "json",
      },
  ...
  const { default: answersList } = await import(
    "./backup_json_data/discord_quiz_answer.json",
    {
      assert: {
        type: "json",
      },
  ...
  const { default: topicsList } = await import(
    "./backup_json_data/topics.json",
    {
      assert: {
        type: "json",
      },
  ...
  const { default: problemsList } = await import(
    "./backup_json_data/problems.json",
    {
      assert: {
        type: "json",
      },
  ...
  await Promise.all(
    problemsList.map(async (problem) => {
      const { id, title, title_slug, difficulty, premium, topics } = problem;
  ...
  const { default: userProblemsList } = await import(
    "./backup_json_data/user_solved_problems.json",
    {
      assert: {
        type: "json",
      },
  ...
  const { default: missionsList } = await import(
    "./backup_json_data/missions.json",
    {
      assert: {
        type: "json",
      },
  ...
  await Promise.all(
    missionsList.map(async (mission) => {
      const { id, name, description, isHidden, rewardImageURL, problems } =
        mission;
  ...
  const { default: dailyObjects } = await import(
    "./backup_json_data/dailies.json",
    {
      assert: {
        type: "json",
      },
  ...
  const { default: userDailyObjects } = await import(
    "./backup_json_data/user_dailies.json",
    {
      assert: {
        type: "json",
      },
  ...
  const { default: userMonthlyObjects } = await import(
    "./backup_json_data/user_monthlies.json",
    {
      assert: {
        type: "json",
      },
  ...
  const createUserMonthlyObject = prisma.userMonthlyObject.createMany({
    data: userMonthlyObjects.map(
      ({ id, userId, scoreEarned, firstDayOfMonth }) => ({
        id,
        userId,
        scoreEarned,
  ...
  const { default: accountList } = await import(
    "./backup_json_data/account.json",
    {
      assert: {
        type: "json",
      },
  ...
  const hashedAccountList = await Promise.all(
  ...
  const createAccounts = prisma.account.createMany({
    data: hashedAccountList.map(({ id, email, password, role }) => ({
      id,
      email,
      password,
      role,
